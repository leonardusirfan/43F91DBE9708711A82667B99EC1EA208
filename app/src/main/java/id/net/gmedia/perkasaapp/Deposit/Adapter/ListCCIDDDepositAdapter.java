...
public class ListCCIDDDepositAdapter extends ArrayAdapter {

    private Activity context;
    private List<CustomItem> items;
    private ItemValidation iv = new ItemValidation();

    public ListCCIDDDepositAdapter(Activity context, List<CustomItem> items) {
    ...
    public void addData(CustomItem item){
        ...
        if(!isExist){
            items.add(0, item);
            notifyDataSetChanged();
            ((DetailCCIDDeposit) context).updateTotal();
        }
