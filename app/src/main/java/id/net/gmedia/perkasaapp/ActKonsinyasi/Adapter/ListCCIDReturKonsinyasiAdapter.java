...
public class ListCCIDReturKonsinyasiAdapter extends ArrayAdapter{

    private Activity context;
    private List<CustomItem> items;
    private ItemValidation iv = new ItemValidation();

    public ListCCIDReturKonsinyasiAdapter(Activity context, List<CustomItem> items) {
    ...
    public void addData(CustomItem item){
        ...
        if(!isExist){
            items.add(0, item);
            notifyDataSetChanged();
            ((DetailReturKonsinyasi) context).updateTotal();
        }
