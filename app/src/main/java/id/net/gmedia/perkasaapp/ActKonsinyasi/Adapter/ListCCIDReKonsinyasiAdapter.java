...
public class ListCCIDReKonsinyasiAdapter extends ArrayAdapter{

    private Activity context;
    private List<CustomItem> items;
    private ItemValidation iv = new ItemValidation();

    public ListCCIDReKonsinyasiAdapter(Activity context, List<CustomItem> items) {
    ...
    public void addData(CustomItem item){
        ...
        if(!isExist){
            items.add(0, item);
            notifyDataSetChanged();
            ((DetailRekonsinyasi) context).updateTotal();
        }
